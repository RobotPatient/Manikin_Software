...
TEST(I2CWrapperTest, initCallsRightMethods) {
    ...
    i2c_driver driver_handle = i2c_driver(&i2c_peripheral_mock, i2c_speed_100KHz, Mock_addr);
...
TEST(I2CWrapperTest, read_regCallsRightMethods){
    ...
    EXPECT_CALL(i2c_peripheral_mock, requestFrom(Mock_addr, request_amount_of_bytes));
    EXPECT_CALL(i2c_peripheral_mock, read()).WillRepeatedly(Return(data_to_return));
    /* The object method which calls to mock methods under the hood*/
...
TEST(I2CWrapperTest, read_reg16CallsRightMethods){
    ...
    {
        InSequence seq;
        EXPECT_CALL(i2c_peripheral_mock, read()).WillOnce(Return(data_upper_byte));
        EXPECT_CALL(i2c_peripheral_mock, read()).WillOnce(Return(data_lower_byte));
    }
    /* The object method which calls to mock methods under the hood*/
